...
**Interpretation limits:** `encoder_counts` are unsigned 16-bit raw values, not degrees. `home_switch_bits` is an undecoded controller byte. The UTC timestamp is assigned by the host when `get_state()` reads the shared buffer; it does not prove a fresh USB packet arrived for every sample. Repeated identical samples are therefore inconclusive until packet freshness is validated. The `homed` flag reflects Python command history, not an independent switch check.
...
The immediate milestone is a successful idle log and a verified, supervised home and one-degree jog. Share those logs and observations to develop the next calibration script from actual controller behavior.
